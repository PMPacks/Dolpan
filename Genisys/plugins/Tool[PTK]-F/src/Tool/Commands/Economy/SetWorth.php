...
class SetWorth extends BaseCommand{
    ...
    public function __construct(BaseAPI $api){
        ...
        $this->setPermission("tool.setworth");
    }
    ...
    public function execute(CommandSender $sender, $alias, array $args): bool{
        ...
        if(!$sender instanceof Player || count($args) !== 1){
            ...
            $sender->sendMessage(TextFormat::RED . "§c[Lỗi] Please provide a valid worth");
            return false;
        }elseif(($id = $sender->getInventory()->getItemInHand()->getId()) === Item::AIR){
            $sender->sendMessage(TextFormat::RED . "§c[Lỗi] Please provide a valid item");
            return false;
        }

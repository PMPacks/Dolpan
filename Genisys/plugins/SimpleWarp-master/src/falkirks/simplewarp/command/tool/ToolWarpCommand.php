<?php
namespace falkirks\simplewarp\command\tool;
...
class EssentialsWarpCommand extends WarpCommand{
    ...
    public function execute(CommandSender $sender, $commandLabel, array $args){
        if(isset($args[0])) {
            ...
            if (isset($this->api->getWarpManager()[$args[0]])) {
                ...
                if($ess->warpExists($args[0]) && $sender->hasPermission("simplewarp.tool.notice")){
    ...
    private function getEssAPI(){
        $ess = $this->getPlugin()->getServer()->getPluginManager()->getPlugin("Tool");

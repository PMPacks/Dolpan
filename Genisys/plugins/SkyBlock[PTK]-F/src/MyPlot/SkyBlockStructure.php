...
class SkyBlockStructure extends Populator{
	...
	public static function placeObject(ChunkManager $level, $chunk, $Xofchunk, $Zofchunk){
		...
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 6, Block::FENCE);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 7, Block::COBBLESTONE);
		$level->setBlockIdAt($vec->x + 8, 69, $vec->z + 8, Block::REEDS);
		$level->setBlockIdAt($vec->x + 6, 69, $vec->z + 7, Block::CRAFTING_TABLE);
		$level->setBlockIdAt($vec->x + 5, 66, $vec->z + 6, Block::FARMLAND);
		$level->setBlockIdAt($vec->x + 5, 67, $vec->z + 6, Block::WHEAT_BLOCK);
